...
mkdir -pv "${SHED_FAKE_ROOT}"/usr/{,local/}share/{color,dict,doc,info,locale,man}
mkdir -v  "${SHED_FAKE_ROOT}"/usr/{,local/}share/{misc,terminfo,zoneinfo}
...
install -v -m644 "${SHED_PKG_CONTRIB_DIR}/passwd" "${SHED_FAKE_ROOT}/etc/passwd.default"
install -v -m644 "${SHED_PKG_CONTRIB_DIR}/group" "${SHED_FAKE_ROOT}/etc/group.default"

case "$SHED_BUILD_MODE" in
    bootstrap)
        # Install default users and groups
        install -v -m644 "${SHED_PKG_CONTRIB_DIR}/passwd" "${SHED_FAKE_ROOT}/etc/passwd"
        install -v -m644 "${SHED_PKG_CONTRIB_DIR}/group" "${SHED_FAKE_ROOT}/etc/group"
